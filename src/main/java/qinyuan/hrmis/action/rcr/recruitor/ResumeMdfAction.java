...
public class ResumeMdfAction extends RecruitorAction {
	...
	public String execute() {
		...
		if (hasParameter("resumeMdfSubmit")) {
			String resumeId = getParameter("resumeId");
			if (isNumeric(resumeId)) {
				...
				if (postId != null && applicant != null && genderId != null
						...
						&& resumeLink != null && isNumeric(postId)
						&& isNumeric(genderId) && isNumeric(degreeId)) {
					recruitor.mdfResume(parseInt(resumeId), parseInt(postId),
							applicant, parseInt(genderId), salary, birthday,
							nativePlace, parseInt(degreeId), school, major,
							resumeLink);
					setAttribute("result", "修改成功");
				}
			}
		}
